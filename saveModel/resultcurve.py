...
class DrawCurves(object):
    ...
    def draw(self, target="train_error"):
        ...
        self.fig_params["label"] = self.fig_params["label"] + "," + str(
            min(input_data))
        x = np.linspace(1, data_len, data_len)
        plt.plot(x,
                 input_data,
                 self.fig_params["line_style"],
                 linewidth=self.fig_params["line_width"],
    ...
    def draw(self):
        ...
        for i in range(len(txt_file_list)):
            ...
            for j in range(len(log_data_list)):
                ...
                plt.hist(input_data)
                plt.grid()
                title_str = "epoch:%d, block:%d, layer:%d" % (
                    log_data_list[j]["epoch"], log_data_list[j]["block"],
                    log_data_list[j]["layer"])
                plt.title(title_str)
                save_path = self.fig_folder + "epoch_%d_block_%d_layer_%d.png" % (
                    log_data_list[j]["epoch"], log_data_list[j]["block"],
                    log_data_list[j]["layer"])
